...
func initApplication(debug bool, serverConf *conf.Server, dbConf *data.Database, cacheConf *data.CacheConf, i18nConf *translator.I18n, swaggerConf *router.SwaggerConfig, serviceConf *service_config.ServiceConfig, uiConf *server.UI, logConf log.Logger) (*pacman.Application, func(), error) {
	...
	siteRankRepo := rank.NewUserSiteRankRepo(dataData)
	rankService := rank2.NewRankService(userCommon, userRankRepo, objService, userRoleRelService, rolePowerRelService, configService, siteRankRepo)
	...
	networkProfileService := site2.NewNetworkProfileService(userCommon, siteRepo, siteRankRepo, networkProfileRepo, networkProjectRepo, profileTagRepo, serviceConf)

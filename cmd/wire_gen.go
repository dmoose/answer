...
import (
	...
	site2 "github.com/apache/answer/internal/service/site"
...
func initApplication(debug bool, serverConf *conf.Server, dbConf *data.Database, cacheConf *data.CacheConf, i18nConf *translator.I18n, swaggerConf *router.SwaggerConfig, serviceConf *service_config.ServiceConfig, uiConf *server.UI, logConf log.Logger) (*pacman.Application, func(), error) {
	...
	siteRoleRepo := role.NewUserSiteRoleRelRepo(dataData)
	userRoleRelService := role2.NewUserRoleRelService(userRoleRelRepo, roleService, siteRoleRepo)
	...
	controllerSiteInfoController := controller.NewSiteInfoController(siteInfoCommonService, serviceConf)
	...
	networkProfileService := site2.NewNetworkProfileService(userCommon, siteRepo, networkProfileRepo, networkProjectRepo, profileTagRepo, serviceConf)
	siteController := controller.NewSiteController(siteService, networkProfileService, serviceConf)
	...
	memberDirectoryService := network_directory.NewMemberDirectoryService(memberDirectoryRepo, profileTagRepo, siteInfoCommonService)
	memberDirectoryController := controller.NewMemberDirectoryController(profileEditService, profileTagService, memberDirectoryService, serviceConf)
	profileTagAdminController := controller_admin.NewProfileTagAdminController(profileTagService, serviceConf)
	...
	authUserMiddleware := middleware.NewAuthUserMiddleware(authService, siteInfoCommonService, userRoleRelService)
	...
	connectorController := controller.NewConnectorController(siteInfoCommonService, emailService, userExternalLoginService, siteRepo)
	...
	ginEngine := server.NewHTTPServer(debug, staticRouter, answerAPIRouter, swaggerRouter, uiRouter, authUserMiddleware, avatarMiddleware, shortIDMiddleware, siteMiddleware, templateRouter, pluginAPIRouter, uiConf)
	scheduledTaskManager := cron.NewScheduledTaskManager(siteInfoCommonService, questionService, fileRecordService, userAdminService, serviceConf, siteService)
